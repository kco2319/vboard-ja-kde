...
# --- JIS-only keys not always predefined in python-uinput; fall back to raw (name, code) tuples ---
try:
...
try:
    ...
    KEY_RO = ("KEY_RO", 89)  # must stay distinct from KEY_BACKSLASH (already used by the "]" button)

# NOTE: labels below are the JIS (pc106) *unshifted* character each keycode
# actually produces under the "jp" XKB layout set in ~/.config/kxkbrc.
# This intentionally differs from the raw US key names in several spots
# (KEY_LEFTBRACE -> "@", KEY_RIGHTBRACE -> "[", KEY_BACKSLASH -> "]",
#  KEY_APOSTROPHE -> ":", KEY_EQUAL -> "^"), because JIS keyboards put those
# characters on different physical keys than a US keyboard does.
...
class VirtualKeyboard(Gtk.Window):
    def __init__(self):
        ...
        self.header.set_decoration_layout(":minimize")
        ...
        # Define rows for keys (relabeled to match JIS keyboard positions).
        # Esc/F1-F12 plus the JIS-only ¥ and \ keys share the top row.
        rows = [
    ...
    def create_settings(self):
        ...
        # Order matters: first pack_end() call ends up at the outer (rightmost)
        # edge, so pack close first, then maximize, then resize.
        self.create_longpress_button("✕", self.do_close, tooltip="長押しで閉じる")
        self.create_longpress_button("□", self.do_maximize_toggle, tooltip="長押しで最大化/元に戻す")
    ...
    def create_longpress_button(self, label_, action, tooltip=None, hold_ms=600):
        # Fires `action` only after the button is held for hold_ms — a plain
        # tap does nothing. This guards against accidental close/maximize
        # taps on a touchscreen-only device.
        button = Gtk.Button(label=label_)
        button.set_name("headbar-button")
        if tooltip:
            button.set_tooltip_text(tooltip)

        state = {"timer": None, "fired": False}
        ...
        def on_pressed(widget):
            state["fired"] = False
            state["timer"] = GLib.timeout_add(hold_ms, fire)

        def cancel(widget, *args):
            if state["timer"] is not None:
                GLib.source_remove(state["timer"])
                state["timer"] = None

        button.connect("pressed", on_pressed)
        button.connect("released", cancel)
        button.connect("leave-notify-event", cancel)
    ...
    def create_resize_button(self):
        # Drag-based resize turned out unreliable on this hardware: taps
        # register (press event fires) but no motion-notify-event ever
        # follows, meaning the touchscreen isn't being emulated as a
        # draggable pointer under XWayland here — only discrete taps.
        # So: step-resize instead, using the same tap pattern that already
        # works reliably for the opacity +/- buttons.
        self.create_button("⤢+", self.resize_step_cb, 40, 2)
        self.create_button("⤢-", self.resize_step_cb, -40, 2)

    def resize_step_cb(self, widget, delta):
        width, height = self.get_size()
        new_w = max(200, width + delta)
        new_h = max(150, height + delta)
        self.resize(new_w, new_h)

    def change_visibility(self, widget=None):
        for button in self.buttons:
            label = button.get_label() if hasattr(button, "get_label") else None
            if label != "☰":
    ...
    def apply_css (self):
        ...
        headerbar button label, #headbar-button label {{
    ...
    def update_label(self, show_symbols):
        # (row_button index, "base shifted") pairs — recomputed for the JIS
        # relabeling above. Index 0 (`) and 10 (0) are omitted: JIS defines
        # no usable shifted symbol for either, so those buttons don't toggle.
        button_positions = [
            (13, "¥ |"), (14, "\\ _"),
